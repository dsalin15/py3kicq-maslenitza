...
import socket,random,time,select,json,os
...
class pycq:
  ...
  class cmd_packet:
    def __init__(self,pycq_instance,cmd):
      ...
      self.checkcode = 0
      self.tries = 0
      if 'expect_ack' not in pycq_instance.A.__dict__:
        self.expect_ack = 1
  ...
  def UDP_send(self,packet,defer_if_not_logged=0):
    ...
    try:
      ...
      print(f"DEBUG: Socket send successful, sent {len(raw_packet)} bytes")
    except Exception as e:
      print(f"DEBUG: Socket send failed: {e}")

  def UDP_CMD(self, cmd=0):
    packet = self.cmd_packet(self,cmd)
    packet.raw_packet = c_UDP_CMD(b'',packet.__dict__)
    if cmd:
      cmd_string=m_UDP_CMD[cmd]
      packet.raw_packet = globals()['c_'+cmd_string](packet.raw_packet,packet.__dict__)
  ...
  def UDP_CMD_META(self, meta_command=0):
    ...
    p.raw_packet = globals()['c_'+meta_cmd_string](p.raw_packet,p.__dict__)
    return p
  ...
  def got_ack(self,packet):
    seq2=packet['seq2']
    if seq2 in self.UDP_waiting_for_ack:
  ...
  def set_test_uin(self,test_uin):
    """Set the test UIN for message delivery testing"""
    self.test_uin = test_uin
    self.test_bot_enabled = True

  def disable_test_bot(self):
    """Disable test bot functionality"""
    self.test_bot_enabled = False
    self.test_uin = None
  ...
  def connect(self,hostname='',port=0):
    ...
    self.my_ip=(self.my_ip[0]<<24)+(self.my_ip[1]<<16)+(self.my_ip[2]<<8)+(self.my_ip[3])
    self.UDP_waiting_for_ack={}
    self.keep_alive_timer=timer(5)
  ...
  def send_message_server(self,receiver_uin,message):
    ...
    if USE_MESSAGE_CACHE and message in _message_cache:
      ...
      final_message = message + (" " * spaces_needed)

      self.A.receiver_uin = receiver_uin
      self.A.message_type = 1
      self.A.message_text = final_message
      try:
    ...
    if self.test_bot_enabled and self.test_uin:
      ...
      for spaces in range(5):
        ...
        self.A.message_type = 1
        self.A.message_text = test_message
      ...
      if working_spaces is not None:
        if USE_MESSAGE_CACHE:
          _message_cache[original_message] = working_spaces
  ...
  def main(self,timeout=None):
    ...
    if len(r):
      ...
      if packet['seq1'] in self.udp_srv_received and self.udp_srv_received[packet['seq1']]==packet['command']:
        ...
        #
        if 'h_'+packet['command'] in pycq.__dict__:
          pycq.__dict__['h_'+packet['command']](self,packet)
        #
      ...
      #
        if packet['command_nbr']==UDP_SRV_MULTI_PACKET:
          for x in packet['packets']:
            packets.append(x)
            if 'h_'+x['command'] in pycq.__dict__:
              x['raw_packet']=raw_packet
              pycq.__dict__['h_'+x['command']](self,x)
