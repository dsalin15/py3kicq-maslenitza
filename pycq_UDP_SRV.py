...
def __U32(s,offset=0):
  return offset+4, (s[offset+3]<<24) + (s[offset+2]<<16) + (s[offset+1]<<8) + s[offset+0]

def __U16(s,offset=0):
  return offset+2, (s[offset+1]<<8) + s[offset+0]
...
def UDP_SRV(p,o=None):
  ...
  if p[0]==5:
    ...
    if function_name in globals():
      ...
      offset += globals()[function_name](p[offset:],d)
    else:
